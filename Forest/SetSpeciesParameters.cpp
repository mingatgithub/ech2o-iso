...
int Forest::SetSpeciesParameters(Control &ctrl){
  ...
  for(UINT4 i = 0; i < _tableParams->r; i++){
    ...
    _species[i].gs_lwp_low = _tableParams->matrix[i][14];
    _species[i].gs_lwp_high = _tableParams->matrix[i][15];
    ...
    _species[i].vegtype = _tableParams->matrix[i][37];
    ...
    if(_species[i].vegtype==1){
      _species[i].Fprn = _tableParams->matrix[i][38]; //This reassigns dead grass leaf turnover rate to Fprn if vegtype=1.
      _species[i].Fpra = _tableParams->matrix[i][39];//This reassign dead grass leaf turnover adjustment rate to reuse Fpra if vegtype =1
    }
