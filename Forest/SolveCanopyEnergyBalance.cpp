...
UINT4 Forest::SolveCanopyEnergyBalance(Basin &bas, Atmosphere &atm, Control &ctrl,
  ...
  REAL8 dleafRHdT = 0;
  REAL8 dleafRHdpsi_l = 0; // [[maybe_unused]]
  //REAL8 dgcdlwp = 0;
  REAL8 dEdlwp = 0;
  REAL8 dEdT = 0;
  REAL8 dLETdlwp =0;
  ...
  REAL8 gc = 0;
  REAL8 lwp_high, lwp_low; //high and low lwp for stomatal model
  REAL8 dgcdfgspsi = 0;
  REAL8 dgs_psidpsi = 0;
  ...
  if (s == nsp - 1) //for bare soil, water reaching the ground is pp times its proportion of the cell
    ...
    lwp_low = _species[s].gs_lwp_low;
    lwp_high = _species[s].gs_lwp_high;
    ...
    //calculates leaf surface relative humidity 
    leavesurfRH = airRH + ((1 - airRH) / MaxCanStor) * CanStor;
    //leavesurfRH = CanStor > RNDOFFERR ? 1 : airRH + ((1 - airRH) / MaxCanStor) * CanStor;
