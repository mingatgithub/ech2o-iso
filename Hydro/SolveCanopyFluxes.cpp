...
int Basin::SolveCanopyFluxes(Atmosphere &atm, Control &ctrl, Tracking &trck) {
  ...
  REAL8 snow_rain_thres = 0; //temperature threshold for snow rain transition, degC
  ...
  REAL8 d2Hcanopy_f, d18Ocanopy_f, cClcanopy_f, Agecanopy_f;
  REAL8 kTB_L1, kTB_L2;
  REAL8 kMW_L1, kMW_L2;
  ...
  _FluxSnowmelt->reset(); // snowmelt flux
  if(ctrl.sw_trck){
    //For tracking
...
#pragma omp parallel default(none)					\
  private( s, r,c, p,  treeheight, wind, za, z0o, zdo,			\
	   Tp, maxTp, minTp, snow, rain, snow_rain_thres, evap,		\
	   transp, LE, H, netR, evap_f, transp_f, D, DelCanStor, \
	   theta, theta2, theta3, theta_available, ra,			\
	   ...
	   d2HevapI_f, d18OevapI_f, AgeevapI_f,				\
	   d2Hcanopy_f, d18Ocanopy_f, cClcanopy_f, Agecanopy_f,		\
...
#pragma omp for nowait
	...
	if (s == nsp - 1) { //if this is bare ground set D to precip and skip the tree stuff
	  ...
	  //cout << "theta_a : " << theta_available << ", theta : " << theta <<
	  ...
	  */
	  ...
	  if (ctrl.sw_trck and DelCanStor > RNDOFFERR){
	    if(ctrl.sw_2H)
	      fForest->setd2Hcanopy(s, r, c,
				    fForest->getIntercWater(s, r, c) <= RNDOFFERR ? sqrt(-1) :
	    ...
	    if(ctrl.sw_Cl)
	      fForest->setcClcanopy(s, r, c,
				    fForest->getIntercWater(s, r, c) <= RNDOFFERR ? sqrt(-1) :
				    trck.InputMix(fForest->getIntercWater(s, r, c) - DelCanStor,
						  fForest->getcClcanopy(s)->matrix[r][c],
						  DelCanStor,
						  atm.getcClprecip()->matrix[r][c]));
	  ...
	  if(ctrl.sw_trck){
	    ...
	    if(ctrl.sw_Cl)
	      cClcanopy_f += fForest->getcClcanopy(s)->matrix[r][c]*p*fForest->getIntercWater(s, r, c);
	    if(ctrl.sw_Age)
	  ...
	  if(ctrl.sw_trck){
	    if(ctrl.sw_2H){
	      fForest->setd2HevapI(s, r, c,
				   evap <= RNDOFFERR ? sqrt(-1) : fForest->getd2Hcanopy(s)->matrix[r][c]);
	      d2HevapI_f += fForest->getd2HevapI(s)->matrix[r][c] * p * evap ;
	    }
	    if(ctrl.sw_18O){
	      fForest->setd18OevapI(s, r, c,
				    evap <= RNDOFFERR ? sqrt(-1) : fForest->getd18Ocanopy(s)->matrix[r][c]);
	      d18OevapI_f += fForest->getd18OevapI(s)->matrix[r][c] * p * evap ;
	    }
	    if(ctrl.sw_Age){
	      fForest->setAgeevapI(s, r, c,
				   evap <= RNDOFFERR ? sqrt(-1) : fForest->getAgecanopy(s)->matrix[r][c]);
	  ...
	  if(ctrl.sw_trck){
		...
		d18OevapT_f += fForest->getd18OevapT(s)->matrix[r][c] * p * transp ;
	      ...
	      ///// TO BE UPDATED ===================================================
	      // Assuming chloride is not taken up by roots -> concentration in soils
	      if(ctrl.sw_Cl){
		if(dth1 > RNDOFFERR){
		  trck.setcClsoil1(r, c,
				   trck.getcClsoil1()->matrix[r][c] * (1+ dth1/theta));
		}
		if(dth2 > RNDOFFERR)
		  trck.setcClsoil2(r, c,
				   trck.getcClsoil2()->matrix[r][c] * (1+ dth2/theta2));
		if(dth3 > RNDOFFERR){
		  trck.setcClsoil3(r, c,
				   trck.getcClsoil3()->matrix[r][c] * (1+ dth3/theta3));
		  trck.setcClgroundwater(r, c, trck.getcClsoil3()->matrix[r][c]);
		}
	      }
	      //////////// ===========================================================
	      if(ctrl.sw_Age){
		...
		d18OevapT_f += fForest->getd18OevapT(s)->matrix[r][c] * p * transp ;
	      ...
	      // Assuming chloride is not taken by roots -> concentration in soils
	      if(ctrl.sw_Cl){
		if(dth1 > RNDOFFERR)
		  trck.setcClsoil1(r, c,
				   trck.getcClsoil1()->matrix[r][c] * (1+ dth1/theta));
	...
	snow_rain_thres = atm.getSnowRainTempThreshold()->matrix[r][c];

	if(maxTp <= snow_rain_thres){
	  snow = D;
	  rain = 0;
	}
	else if(minTp > snow_rain_thres){
	  rain = D;
	  snow = 0;
	}
	else{
	  snow = D * max<REAL8>(0.0, (snow_rain_thres - minTp) /(maxTp - minTp));
	  rain = D - snow;
	}
	...
	if(ctrl.sw_trck){
	  ...
	  _FluxCnptoSrf->matrix[r][c] += rain * p ;
	  _FluxCnptoSnow->matrix[r][c] += snow * p ;
	...
	trck.setd2HevapT_sum(r, c, transp_f > RNDOFFERR ? d2HevapT_f / transp_f : sqrt(-1));
	trck.setd2HevapI_sum(r, c, evap_f > RNDOFFERR ? d2HevapI_f / evap_f : sqrt(-1));
	trck.setd2Hcanopy_sum(r, c,_CanopyStorage->matrix[r][c] > RNDOFFERR ?
			      d2Hcanopy_f / _CanopyStorage->matrix[r][c] : sqrt(-1));
      }
      if(ctrl.sw_trck && ctrl.sw_18O){
	trck.setd18OevapT_sum(r, c, transp_f > RNDOFFERR ? d18OevapT_f / transp_f : sqrt(-1));
	trck.setd18OevapI_sum(r, c, evap_f > RNDOFFERR ? d18OevapI_f / evap_f : sqrt(-1));
	trck.setd18Ocanopy_sum(r, c, _CanopyStorage->matrix[r][c] > RNDOFFERR ?
			       d18Ocanopy_f / _CanopyStorage->matrix[r][c] : sqrt(-1));
      }
      if(ctrl.sw_trck && ctrl.sw_Cl)
	trck.setcClcanopy_sum(r, c, _CanopyStorage->matrix[r][c] > RNDOFFERR ?
			      cClcanopy_f / _CanopyStorage->matrix[r][c] : sqrt(-1));
      if(ctrl.sw_trck && ctrl.sw_Age){
	trck.setAgeevapT_sum(r, c, transp_f > RNDOFFERR ? AgeevapT_f / transp_f : sqrt(-1));
	trck.setAgeevapI_sum(r, c, evap_f > RNDOFFERR ? AgeevapI_f / evap_f : sqrt(-1));
	trck.setAgecanopy_sum(r, c, _CanopyStorage->matrix[r][c] > RNDOFFERR ?
			      Agecanopy_f / _CanopyStorage->matrix[r][c] : sqrt(-1));
      }
