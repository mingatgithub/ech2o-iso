...
class Basin {
  ...
  grid *_KsatL1, *_KsatL2, *_KsatL3 ; // Horizontal saturated hydraulic conductivities (m s-1)
  grid *_kKsat; // coefficient for exponential profile of Ksat, in m
  grid *_KvKs; //vertical to horiontal Ks anisotropy ratio
  grid *_bedrock_leak; // Fraction of _KsatL3*KvKs for deep drainage / leakage (0-1)
  ...
  grid *_ponding; // non-channel water ponding on the soil surface in m
  grid *_Schannel; // channel storage in m
  grid *_Ssurface ; // sum of ponding  and channel storages
  ...
  grid *_Leakage; //water flux down the bottom of the profile (out of the domain) m.s-1
  ...
  grid *_GrndWater2; //water stored along the continuum gw system from the surface (e.g. perched aquifers, m)
  ...
  grid *_netrad_tot; // Summed net radiation over the surface-vegetation continuum Wm-2
  ...
  grid *_latheat_tot; // Total latent heet from the surface-vegetation continuum Wm-2
  ...
  grid *_sensheat_tot; // Total sensible heat from the surface-vegetation continuum Wm-2
  ...
  grid *_FluxThroughfall; // thoughfall (both liquid and snow) -> m s-1
  grid *_FluxSnowmelt; // snowpack to surface -> m s-1
  grid *_FluxInfilt; // timestep-cumulated infiltration from first layer -> m s-1
  grid *_FluxPercolL2; // timestep-cumulated percolation L1 to L2 -> m s-1
  grid *_FluxPercolL3; // timestep-cumulated percolation L2 to L3 -> m s-1
  grid *_FluxRecharge; // timstep-cumulated recharge to L3's GW -> m s-1
  // (for tracking/mixing only)
  grid *_FluxCnptoSrf; // canopy/sky to surface -> m s-1
  grid *_FluxCnptoSnow; // canopy/sky to snowpack -> m s-1
  grid *_FluxSrftoL1; // infiltration from first layer (changes twice during time step) -> m s-1
  grid *_FluxL1toL2; // percolation L1 to L2 -> m s-1
  grid *_FluxL2toL3; // percolation L2 to L3 -> m s-1
  // Vertical (intra-cell) fluxes upwards
  grid *_FluxExfilt; // first layer to surface (return flow) -> m s-1
  grid *_FluxL2toL1; // capillary + return flow, L2 to L1 -> m s-1
  grid *_FluxL3toL2; // capillary + return flow, L2 to L3 -> m s-1
  grid *_FluxTranspiL1; // Transpiration withdrawal from L1 -> m s-1
  grid *_FluxTranspiL2; // Transpiration withdrawal from L2 -> m s-1
  grid *_FluxTranspiL3; // Transpiration withdrawal from L3 -> m s-1
  // Other intra-cell fluxes
  grid *_FluxGWtoChn; // groundwater seepage to channel -> m s-1
  grid *_FluxSrftoChn; // overland flow to channel -> m s-1
  ...
  // Cumulated fluxes, in meters, for map report only
  ...
  grid *_AccLeakage; // timstep-cumulated deep drainage out of the system
  ...
  grid *_AccEvaporationI; // Evaporated interception
  grid *_AccEvaporationS; // Soil evaporation
  grid *_AccEvaporationT; // Transpiration
  ...
  int CalcInitialStreamStorage(Control &ctrl);
  int CalcRootDistrib();
...
public:
  ...
  grid *getSensHeat_tot() const {
    return _sensheat_tot;
  }
...
#pragma omp parallel for			\
  default(none) private(r,c, d3)
    for (unsigned int j = 0; j < _vSortedGrid.cells.size(); j++) {
      ...
      _SoilWaterDepthL3->matrix[r][c] = _soilmoist3->matrix[r][c] * d3;
    }	  
  ...
  grid *getInitGroundwater() const {
    int r, c;
    double fc1, fc2, fc3, d3;
#pragma omp parallel for			\
  default(none) private(r,c, fc1, fc2, fc3, d3)
    for (unsigned int j = 0; j < _vSortedGrid.cells.size(); j++) {
      r = _vSortedGrid.cells[j].row;
      c = _vSortedGrid.cells[j].col;
	...
	max<double>(0,_soilmoist1->matrix[r][c] - fc1) * _depth_layer1->matrix[r][c] +
	max<double>(0,_soilmoist2->matrix[r][c] - fc2) * _depth_layer2->matrix[r][c] +
	max<double>(0,_soilmoist3->matrix[r][c] - fc3) * d3;
    }	  
    return _GrndWater;
  }
  ...
  grid *getFluxSnowmelt() const {
    return _FluxSnowmelt;
  }
  ...
  // Cl-
  grid *getcClcanopy(UINT4 n) const ;
